 ...
 */
function createBootstrappedEngine(persistedWorkspace) {
  ...
  // Ensure the default framework is assigned
  const hasFw = engine.toJSON().frameworks.some(
    (f) => f.organizationId === DEFAULT_ORG_ID && f.frameworkId === DEFAULT_FRAMEWORK_ID
  );
  if (!hasFw) {
    try {
...
export function useOrganizationStore() {
  ...
  const [workspaceData, setWorkspaceData] = useState(() => loadLegacyWorkspace());

  // Build (or rebuild) engine from the persisted snapshot
  const engine = useMemo(
    () => createBootstrappedEngine(engineSnapshot),
    [engineSnapshot]
  );
  ...
  useEffect(() => {
    const handleWorkspaceUpdate = () => {
      setWorkspaceData(loadLegacyWorkspace());
  ...
  const saveWorkspaceItem = useCallback(
    (itemId, state) => {
      ...
      const nextWorkspaceData = { ...latestWorkspace, [itemId]: state };
 ...
 */
export function loadOrgQuestionnaireAnswers() {
  try {
 ...
 */
export function saveOrgQuestionnaireAnswers(answers) {
